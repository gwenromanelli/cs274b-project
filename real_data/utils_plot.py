...
def plot_credibility_interval(p_values, file_name, n, n_values=5):
    ...
    for p_value in tqdm.tqdm(p_values):
        ...
        breakpoint()
        samples = samples.reshape((-1, P, Q), order='F')
        indices = np.tril_indices(P, k=-1)
        samples_tril = samples[:,indices[0], indices[1]]
        samples_rect = samples[:,:,:].reshape(-1, P * Q)
    ...
    flow_samples = [pd.DataFrame(samples_dict[p_value][:,idx_median]).assign(model=f"CMF (q={p_value})")  #,columns=pair_vars[idx_median]
